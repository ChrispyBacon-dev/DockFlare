    ...
    const loadMailboxes = async () => {
        ...
        try {
            ...
            if (decoded?.role === 'user') {
                ...
                store.mailboxes = addresses.map((addr) => ({ address: addr, display_name: '' }));
                const prefs = await Promise.allSettled(addresses.map(addr => mailApi.getMailboxPreferences(addr)));
