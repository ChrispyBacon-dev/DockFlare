// src/components/effects/BluePortalEffect.tsx - PORTAL ! :)
'use client';
...
const BLUE_THEME_COLORS = ['#1e40af', '#2563eb', '#60a5fa'];

const BluePortalEffect: React.FC<PortalEffectProps> = ({ isVisible, size = 40 }) => { 
  ...
  useEffect(() => {
    ...
    if (!canvas || !ctx) return cleanupAnimation;
    ...
    if (containersRef.current.length === 0) {
      for (let i = 0; i < MINI_PORTAL_CONFIG.NUM_CONTAINERS_PER_EFFECT; i++) {
        ...
        }, i * MINI_PORTAL_CONFIG.STAGGER_SPAWN_DELAY);
  ...
  return (
    <div 
      className={`${styles.portalBase} ${isVisible ? styles.portalVisible : styles.portalHidden} ${styles.tableBluePortal}`}
...
export default BluePortalEffect;