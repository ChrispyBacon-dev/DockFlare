...
class QueueLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        try:
            ...
            except queue.Full: print("Log queue full, dropping message.", file=sys.stderr)
...
def process_container_start(container):
    ...
    try:
        ...
        except NotFound: logging.warning(f"Container {container_name} ({container_id[:12]}) not found processing start (stopped quickly?)."); return
        ...
        with state_lock:
            ...
            if existing_rule:
                ...
                if existing_rule.get("status") == "pending_deletion": logging.info(f"Rule for {hostname} was pending deletion. Reactivating."); existing_rule["status"] = "active"; existing_rule["delete_at"] = None; existing_rule["service"] = service; existing_rule["container_id"] = container_id; existing_rule["zone_id"] = target_zone_id; state_changed_locally = True; needs_cf_update = True; if zone_id_changed: logging.info(f"Zone ID for {hostname} updated to {target_zone_id}.")
                    ...
                    if container_changed: logging.info(f"Updating container ID for {hostname}: ... -> {container_id[:12]}."); existing_rule["container_id"] = container_id; state_changed_locally = True
                    if service_changed: logging.info(f"Updating service for {hostname}: '{existing_rule.get('service')}' -> '{service}'."); existing_rule["service"] = service; state_changed_locally = True; needs_cf_update = True
                    if zone_id_changed: logging.warning(f"Zone ID for active rule {hostname} changed ('{existing_rule.get('zone_id')}' -> '{target_zone_id}'). DNS record might be stale in old zone."); existing_rule["zone_id"] = target_zone_id; state_changed_locally = True
            ...
            if state_changed_locally: logging.debug(f"Saving state after processing {hostname}."); save_state()
        if needs_cf_update:
        ...
        elif state_changed_locally: logging.debug(f"Local state updated for {hostname}, no Cloudflare config change needed.")
...
def schedule_container_stop(container_id):
    ...
    with state_lock:
        ...
        if hostname_to_schedule:
            ...
            if rule.get("status") != "pending_deletion": rule["status"] = "pending_deletion"; rule["delete_at"] = datetime.now(timezone.utc) + timedelta(seconds=GRACE_PERIOD_SECONDS); logging.info(f"Rule {hostname_to_schedule} scheduled for deletion at {rule['delete_at'].isoformat()}"); state_changed = True
        ...
        try:
            ...
            if rules_to_delete:
                ...
                for hostname, zone_id in rules_to_delete.items():
                    ...
                    else: logging.error(f"Cannot delete DNS for {hostname}: Missing Tunnel ID."); dns_delete_success_all = False
                if processed_hostnames_for_cf_update:
                    ...
                    with state_lock:
                        ...
                        for hostname in processed_hostnames_for_cf_update:
                            ...
                            else: logging.warning(f"Rule {hostname} scheduled for removal but not found/pending when removing from state.")
                        logging.info(f"Removed {deleted_count} rules from local state.");
                        if state_changed_in_cleanup: save_state()
                    else: logging.error("Failed to update Cloudflare tunnel config during rule cleanup. Rules remain in local state. Will retry.")
                else: logging.info("No hostnames eligible for tunnel config update after DNS processing.")
...
def reconcile_state():
    ...
    try:
             ...
             for c in containers:
                 try:
                     ...
                     if enabled and hostname and service:
                         ...
                         if hostname in running_labeled_containers: logging.warning(f"[Reconcile] Duplicate hostname '{hostname}' on {container_name} & {running_labeled_containers[hostname]['container_name']}. Using {container_name}.")
        ...
        with state_lock:
            now_utc = datetime.now(timezone.utc); managed_hostnames = set(managed_rules.keys()); running_hostnames = set(running_labeled_containers.keys()); hostnames_requiring_dns_check = []
            for hostname, running_details in running_labeled_containers.items():
                ...
                if not target_zone_id: logging.error(f"[Reconcile] Skipping management for {hostname}: No valid Zone ID determined."); continue
                if hostname in managed_rules:
                    rule = managed_rules[hostname]; zone_id_changed = rule.get("zone_id") != target_zone_id
                    if rule.get("status") == "pending_deletion": logging.info(f"[Reconcile] Hostname {hostname} running, reactivating pending rule."); rule["status"] = "active"; rule["delete_at"] = None; rule["service"] = running_details["service"]; rule["container_id"] = running_details["container_id"]; rule["zone_id"] = target_zone_id; state_changed_locally = True; needs_cf_update = True; hostnames_requiring_dns_check.append(hostname); if zone_id_changed: logging.info(f"[Reconcile] Zone ID for {hostname} updated to {target_zone_id}.")
                        ...
                        if zone_id_changed: logging.warning(f"[Reconcile] Zone ID for active rule {hostname} changed ('{rule.get('zone_id')}' -> '{target_zone_id}')."); rule["zone_id"] = target_zone_id; state_changed_locally = True; hostnames_requiring_dns_check.append(hostname)
            ...
            for hostname in list(managed_hostnames):
                if hostname not in running_hostnames and hostname in managed_rules and managed_rules[hostname].get("status") == "active": logging.info(f"[Reconcile] Rule {hostname} active but no container running. Scheduling deletion."); managed_rules[hostname]["status"] = "pending_deletion"; managed_rules[hostname]["delete_at"] = now_utc + timedelta(seconds=GRACE_PERIOD_SECONDS); state_changed_locally = True
            ...
            if current_cf_config is not None:
                ...
                if cf_ingress_hostnames != active_managed_hostnames: logging.warning(f"[Reconcile] Mismatch: Managed rules ({len(active_managed_hostnames)}) vs CF config ({len(cf_ingress_hostnames)})!"); logging.info(f"[Reconcile] Managed: {sorted(list(active_managed_hostnames))}"); logging.info(f"[Reconcile] Cloudflare: {sorted(list(cf_ingress_hostnames))}"); needs_cf_update = True
            ...
            if update_cloudflare_config():
                 ...
                 for hostname in hostnames_requiring_dns_check:
                     ...
                     if rule_details and rule_details.get("zone_id") and tunnel_state.get("id"):
                         if not create_cloudflare_dns_record(rule_details["zone_id"], hostname, tunnel_state["id"]): logging.error(f"[Reconcile] CRITICAL: Failed DNS check/create for {hostname} in zone {rule_details['zone_id']} after tunnel update.")
                     else: logging.error(f"[Reconcile] Cannot check/create DNS for {hostname}: Missing rule details, zone ID, or tunnel ID.")
...
def update_cloudflared_container_status():
    ...
    if container:
        ...
        if cloudflared_agent_state["container_status"] != new_status: logging.info(f"Agent container status -> {new_status}"); cloudflared_agent_state["container_status"] = new_status; if new_status == 'running': cloudflared_agent_state["last_action_status"] = None
...
def ensure_docker_network_exists(network_name):
    ...
    except NotFound: logging.info(f"Network '{network_name}' not found. Creating...");
...
def start_cloudflared_container():
    ...
    try:
        ...
        if not ensure_docker_network_exists(CLOUDFLARED_NETWORK_NAME): logging.error(f"Failed network check for '{CLOUDFLARED_NETWORK_NAME}'. Cannot start agent."); if not cloudflared_agent_state["last_action_status"] or "network" not in cloudflared_agent_state["last_action_status"].lower(): cloudflared_agent_state["last_action_status"] = f"Error: Failed network check for {CLOUDFLARED_NETWORK_NAME}"; return False
        ...
        if container:
             try:
                 ...
                 if needs_recreate: logging.info(f"Removing misconfigured container '{CLOUDFLARED_CONTAINER_NAME}'...");
                 ...
                 else: logging.info(f"Starting existing container '{CLOUDFLARED_CONTAINER_NAME}'..."); container.start(); msg = f"Started existing container."; cloudflared_agent_state["last_action_status"] = msg; logging.info(msg); success_flag = True
        ...
        if not container and not success_flag:
            ...
            try:
                ...
                new_container = docker_client.containers.run(**container_params); msg = f"Created/started container '{new_container.name}' ({new_container.id[:12]})."; cloudflared_agent_state["last_action_status"] = msg; logging.info(msg); success_flag = True
            except APIError as create_err:
                if "is already in use" in str(create_err): logging.error(f"Container name '{CLOUDFLARED_CONTAINER_NAME}' conflict."); msg = f"Error: Container name conflict. Remove manually & retry."
        ...
        logging.info(f"Exiting start_cloudflared_container (Success: {success_flag})."); return success_flag

def stop_cloudflared_container():
    logging.info(f"Attempting stop agent container '{CLOUDFLARED_CONTAINER_NAME}'..."); cloudflared_agent_state["last_action_status"] = "Stopping..."; success_flag = False
    try:
        ...
        logging.info(f"Exiting stop_cloudflared_container (Success: {success_flag})."); return success_flag
...
def start_tunnel():
    logging.info("UI request: Start tunnel agent."); start_cloudflared_container(); time.sleep(1); return redirect(url_for('status_page'))

@app.route('/stop-tunnel', methods=['POST'])
def stop_tunnel():
    logging.info("UI request: Stop tunnel agent."); stop_cloudflared_container(); time.sleep(1); return redirect(url_for('status_page'))
...
def force_delete_rule(hostname):
    ...
    with state_lock:
        ...
        if rule_details: zone_id_for_delete = rule_details.get("zone_id"); else: logging.warning(f"Rule {hostname} not found in state during force delete. Attempting DNS delete in default zone ID."); zone_id_for_delete = CF_ZONE_ID
    if zone_id_for_delete and tunnel_state.get("id"):
        logging.info(f"Attempting DNS record deletion for force-deleted rule: {hostname} in zone {zone_id_for_delete}"); dns_delete_success = delete_cloudflare_dns_record(zone_id_for_delete, hostname, tunnel_state["id"])
        if not dns_delete_success: logging.error(f"Failed delete DNS for {hostname} in zone {zone_id_for_delete}. Tunnel update proceeding."); cloudflared_agent_state["last_action_status"] = f"Warning: Failed DNS delete for {hostname}. Tunnel update proceeding."
    ...
    with state_lock:
        if hostname in managed_rules: logging.info(f"Force deleting rule for {hostname} from local state."); del managed_rules[hostname]; rule_removed_from_state = True; save_state()
        else: logging.warning(f"Rule '{hostname}' already removed from state when force deleting."); rule_removed_from_state = True
    if rule_removed_from_state:
        logging.info(f"Triggering Cloudflare tunnel config update after force deleting {hostname}.")
        if update_cloudflare_config(): logging.info(f"CF tunnel config update successful after force deleting {hostname}."); status_msg = f"Successfully force deleted rule for {hostname} and updated Cloudflare."; status_msg += " (DNS deletion failed/skipped)." if not dns_delete_success else ""; cloudflared_agent_state["last_action_status"] = status_msg
        else: logging.error(f"CRITICAL: State updated after force delete {hostname}, DNS status: {dns_delete_success}, but tunnel config update FAILED!"); cloudflared_agent_state["last_action_status"] = f"Error: Removed {hostname}, DNS delete: {dns_delete_success}, but FAILED tunnel config update!"
...
def stream_logs():
    @stream_with_context
    def event_stream():
        logging.info("Log stream client connected."); yield f"data: --- Log stream connected ---\n\n"
        try:
            while True: log_entry = log_queue.get(block=True); yield f"data: {log_entry}\n\n"
        except GeneratorExit: logging.info("Log stream client disconnected.")
        finally: pass
...
def run_background_tasks():
    ...
    logging.info("Starting background threads..."); event_thread = threading.Thread(target=docker_event_listener, name="DockerEventListener", daemon=True); cleanup_thread = threading.Thread(target=cleanup_expired_rules, name="CleanupTask", daemon=True); event_thread.start(); cleanup_thread.start()
...
if __name__ == '__main__':
    ...
    if not docker_client: logging.error("Docker client unavailable at startup. Limited functionality."); tunnel_state["status_message"] = "Error: Docker client unavailable."; tunnel_state["error"] = "Failed to connect to Docker daemon."; cloudflared_agent_state["container_status"] = "docker_unavailable"; logging.warning("Skipping tunnel init, reconcile, agent management, background tasks.")
    else:
         ...
         if tunnel_state.get("id") and tunnel_state.get("token"):
             logging.info("Tunnel initialized. Proceeding with reconcile & agent checks."); reconcile_state(); logging.info("Initial reconcile complete.")
             logging.info("Checking agent container status..."); update_cloudflared_container_status()
             if cloudflared_agent_state.get("container_status") != 'running': logging.info("Agent not running, attempting auto-start..."); start_cloudflared_container()
             else: logging.info("Agent container already running.")
             event_thread, cleanup_thread = run_background_tasks()
         else: logging.warning("Tunnel not fully initialized. Skipping reconcile, agent start, background tasks."); if not tunnel_state.get("error"): tunnel_state["status_message"] = "Tunnel setup incomplete."
    ...
    try:
        ...
        app.run(host='0.0.0.0', port=5000, threaded=True) # Use threaded dev server if no waitress
