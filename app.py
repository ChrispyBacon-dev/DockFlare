...
CLOUDFLARED_NETWORK_NAME = (
    os.getenv("CLOUDFLARED_NETWORK_NAME", "cloudflare-net") if not USE_EXTERNAL_CLOUDFLARED else None
)
CLOUDFLARED_CONTAINER_NAME = (
    os.getenv("CLOUDFLARED_CONTAINER_NAME", f"cloudflared-agent-{TUNNEL_NAME}")
...
def load_state():
    ...
    if not os.path.exists(state_dir):
        try:
            ...
            logging.error(
                f"FATAL: Could not create directory for state file {state_dir}: {e}. State persistence will fail."
            )
            managed_rules = {}
            return
        ...
        for hostname, rule in loaded_data.items():
            if rule.get("delete_at") and isinstance(rule.get("delete_at"), str):
                try:
                    ...
                    logging.warning(
                        f"Could not parse delete_at for {hostname}: {rule['delete_at']} Error: {date_err}. Setting to None."
                    )
                    rule["delete_at"] = None
            ...
            if "zone_id" not in rule:
                logging.warning(
                    f"Rule for {hostname} loaded from state is missing 'zone_id'. Will attempt to re-determine on reconcile."
                )
                rule["zone_id"] = None
        ...
        try:
            ...
            if isinstance(response_data, dict) and "success" in response_data:
                if response_data["success"]:
                    ...
                    if (
                        cf_errors
                        and isinstance(cf_errors, list)
                        and len(cf_errors) > 0
                        and isinstance(cf_errors[0], dict)
                    ):
                ...
                logging.warning(
                    f"API response for {method} {url} was valid JSON but missing 'success' field. Status: {response.status_code}. Body: {str(response_data)[:200]}"
                )
                raise requests.exceptions.RequestException(
                    f"Unexpected JSON response format from API. Status: {response.status_code}", response=response
                )
        except json.JSONDecodeError:
            logging.error(
                f"API response for {method} {url} was not valid JSON. Status: {response.status_code}. Body: {response.text[:200]}"
            )
            raise requests.exceptions.RequestException(
                f"Invalid JSON response from API. Status: {response.status_code}", response=response
            )
...
def get_zone_id_from_name(zone_name):
    ...
    try:
        ...
        if results and isinstance(results, list) and len(results) == 1:
            ...
            logging.error(
                f"API returned multiple ({len(results)}) active zones matching name '{zone_name}'. Cannot determine correct zone."
            )
            return None
        else:
    ...
    try:
        ...
        if response_data and response_data.get("success"):
            ...
            if zone_data and isinstance(zone_data, dict) and zone_data.get("name"):
                ...
                logging.error(
                    f"API returned success for zone ID '{zone_id}' but result is missing or malformed: {zone_data}"
                )
                return None
        else:
...
def get_cloudflare_account_email():
    ...
    try:
        ...
        if response_data and response_data.get("success"):
            ...
            logging.warning(
                f"Failed to fetch Cloudflare account email, API call unsuccessful or no success flag. Response: {response_data}"
            )
            return None
    except requests.exceptions.RequestException as e:
    ...
    try:
        ...
        if all_apps and isinstance(all_apps, list):
            for app in all_apps:
                if app.get("domain") == hostname:
                    logging.info(
                        f"Found Access Application ID '{app.get('id')}' for hostname '{hostname}' via full list scan."
                    )
                    return app

                if hostname in app.get("self_hosted_domains", []):
                    logging.info(
                        f"Found Access Application ID '{app.get('id')}' for hostname '{hostname}' (in self_hosted_domains) via full list scan."
                    )
                    return app
...
def create_cloudflare_access_application(
    hostname,
    name,
    session_duration,
    app_launcher_visible,
    ...
    endpoint = f"/accounts/{CF_ACCOUNT_ID}/access/apps"
    payload = _build_access_app_payload(
        hostname,
        name,
        session_duration,
        app_launcher_visible,
    ...
    try:
        ...
        if app_data and app_data.get("id"):
            ...
            logging.error(
                f"Access Application creation for '{hostname}' reported success but no ID in response: {app_data}"
            )
            return None
    except requests.exceptions.RequestException as e:
...
def get_cloudflare_access_application(app_uuid):
    ...
    try:
        ...
        if app_data:
            ...
            logging.warning(
                f"Successfully called API for Access App ID '{app_uuid}', but no result data found. Response: {response_data}"
            )
            return None
    except requests.exceptions.RequestException as e:
...
def update_cloudflare_access_application(
    app_uuid,
    hostname,
    name,
    session_duration,
    ...
    try:
        ...
        if app_data and app_data.get("id"):
            ...
            logging.error(
                f"Access Application update for '{app_uuid}' reported success but no ID in response: {app_data}"
            )
            return None
    except requests.exceptions.RequestException as e:
...
def delete_cloudflare_access_application(app_uuid):
    ...
    try:
        ...
        if response_data and response_data.get("success"):
            deleted_id = (
                response_data.get("result", {}).get("id") if isinstance(response_data.get("result"), dict) else app_uuid
            )
            logging.info(
                f"Successfully submitted deletion for Access Application ID '{deleted_id if deleted_id else app_uuid}'"
            )
            return True
        ...
        logging.warning(
            f"Access Application deletion for '{app_uuid}' API call did not confirm success clearly. Response: {response_data}"
        )
        return False
    except requests.exceptions.RequestException as e:
        if e.response is not None and e.response.status_code == 404:
            logging.warning(
                f"Cloudflare Access Application with ID '{app_uuid}' not found during delete attempt (404). Treating as success."
            )
            return True
...
def initialize_tunnel():
    ...
    logging.info(
        f"API Token available: {'Yes (Token masked for security)' if CF_API_TOKEN else 'No (Missing API token)'}"
    )
    ...
    try:
        ...
        logging.info(
            f"Tunnel init completed. State: ID={tunnel_state.get('id')}, Token Present={bool(tunnel_state.get('token'))}, Error={tunnel_state.get('error')}"
        )
    ...
    try:
        ...
        if response_data and response_data.get("success"):
            ...
            if isinstance(config_data, dict):
                ...
                logging.warning(
                    f"Unexpected type for 'config' field in API response: {type(config_data)}. Result: {result_data}"
                )
                return {}
        else:
...
def create_cloudflare_dns_record(zone_id, hostname, tunnel_id):
    ...
    try:
        ...
        if not acquired:
            logging.error(
                f"Timed out waiting for DNS semaphore - too many concurrent operations. Skipping DNS creation for {hostname}"
            )
        ...
        if existing_record_id:
            if correct_tunnel:
                logging.info(
                    f"DNS record for {hostname} already exists with ID {existing_record_id} and correct tunnel. Using existing record."
                )
                return existing_record_id
            else:
                logging.warning(
                    f"DNS record for {hostname} exists (ID: {existing_record_id}) but points to wrong tunnel. Updating..."
                )
                ...
                try:
                    ...
                    if updated_id:
                        logging.info(
                            f"Successfully updated DNS record for {hostname} to point to correct tunnel. ID: {updated_id}"
                        )
                        return updated_id
                    else:
                        logging.error(
                            f"DNS record update API call for {hostname} reported success but response missing ID"
                        )
                        return existing_record_id
        ...
        try:
            logging.info(
                f"Attempting to create DNS CNAME in zone {zone_id}: Name={record_name}, Content={record_content}, Proxied=True"
            )
            response_data = cf_api_request("POST", endpoint, json_data=payload)
            result = response_data.get("result", {})
            ...
            if new_record_id:
                logging.info(
                    f"Successfully created DNS record for {hostname} in zone {zone_id}. New ID: {new_record_id}"
                )
                return new_record_id
            else:
                logging.error(
                    f"DNS record creation API call for {hostname} reported success but response missing ID: {result}"
                )
                return None
        except requests.exceptions.RequestException as e:
            ...
            if cf_error_code == 81057 or (
                ...
                and (
                    "record already exists" in e.response.text.lower()
                    or "a, aaaa, or cname record with that host already exists" in e.response.text.lower()
                )
            ):
    ...
    try:
        ...
        try:
            ...
            if results and isinstance(results, list):
                ...
                if record_id:
                    logging.warning(
                        f"Found DNS record for {hostname} but it points to {record_content} instead of {expected_content}"
                    )
...
def delete_cloudflare_dns_record(zone_id, hostname, tunnel_id):
    ...
    with dns_semaphore:
        ...
        if not record_id:
            logging.warning(
                f"DNS record for {hostname} in zone {zone_id} (pointing to tunnel {tunnel_id}) not found to delete. Assuming success."
            )
            return True
        ...
        try:
            ...
            if e.response is not None and e.response.status_code == 404:
                logging.warning(
                    f"DNS record {record_id} for {hostname} in zone {zone_id} not found during delete attempt (404). Treating as success."
                )
                return True
            ...
            logging.error(
                f"Unexpected error deleting DNS record {record_id} for {hostname} in zone {zone_id}: {e}", exc_info=True
            )
            return False


def generate_access_app_config_hash(
    policy_type,
    session_duration,
    app_launcher_visible,
    allowed_idps_str,
    auto_redirect_to_identity,
    custom_access_rules_str=None,
):
    config_items = {
...
def _handle_access_policy_from_labels(hostname_config_item, current_rule_in_state):
    ...
    desired_access_app_name_from_label = (
        hostname_config_item["access_app_name"] if hostname_config_item["access_app_name"] else f"DockFlare-{hostname}"
    )
    ...
    if desired_access_policy_type_from_label:
        ...
        if desired_access_policy_type_from_label == "default_tld":
            if current_access_app_id:
                logging.info(
                    f"Label policy for {hostname} is 'default_tld'. Deleting existing Access App {current_access_app_id}."
                )
                if delete_cloudflare_access_application(current_access_app_id):
                    ...
                    logging.error(
                        f"Failed to delete Access App {current_access_app_id} for {hostname} as per label 'default_tld'."
                    )
            ...
            if desired_custom_rules_str_from_label:
                try:
                    ...
                    if not isinstance(cf_access_policies, list):
                        logging.error(
                            f"Parsed 'custom_rules' label for {hostname} is not a list. Reverting to default for {desired_access_policy_type_from_label}."
                        )
                        cf_access_policies = []
                except json.JSONDecodeError as json_err:
                    logging.error(
                        f"Error parsing 'custom_rules' label JSON for {hostname}: {json_err}. Reverting to default for {desired_access_policy_type_from_label}."
                    )
                    cf_access_policies = []
            ...
            if not cf_access_policies:
                if desired_access_policy_type_from_label == "bypass":
                    ...
                    cf_access_policies = [
                        {
                            "name": "Label Default Authenticated Access",
                            "decision": "allow",
                            "include": policy_include_rules,
                        }
                    ]

            allowed_idps_list_for_app = (
                [idp.strip() for idp in desired_allowed_idps_str_from_label.split(",") if idp.strip()]
                if desired_allowed_idps_str_from_label
                else None
            )
            needs_action = False
            if current_access_app_id:
                if (
                    current_access_policy_type_in_state != desired_access_policy_type_from_label
            ...
            if needs_action:
                if current_access_app_id:
                    logging.info(
                        f"Updating Access App {current_access_app_id} for {hostname} based on labels (type: {desired_access_policy_type_from_label})."
                    )
                    updated_app = update_cloudflare_access_application(
                        current_access_app_id,
                        hostname,
                        desired_access_app_name_from_label,
                        desired_session_duration_from_label,
                        desired_app_launcher_visible_from_label,
                    ...
                    if updated_app:
                        ...
                        logging.error(
                            f"Failed to update Access App {current_access_app_id} for {hostname} based on labels."
                        )
                else:
                    logging.info(
                        f"Creating new Access App for {hostname} based on labels (type: '{desired_access_policy_type_from_label}')."
                    )
            ...
            logging.warning(
                f"Unknown access.policy type '{desired_access_policy_type_from_label}' from label for {hostname}. No Access App action taken based on this label."
            )
    else:
        if current_access_app_id:
            logging.info(
                f"No access policy label for {hostname}, but found managed Access App {current_access_app_id}. Deleting it as per label configuration."
            )
            if delete_cloudflare_access_application(current_access_app_id):
                current_rule_in_state["access_app_id"] = None
                ...
                logging.error(
                    f"Failed to delete Access App {current_access_app_id} for {hostname} during label-based cleanup."
                )
...
def process_container_start(container):
    ...
    try:
        ...
        default_access_app_launcher_visible_label = labels.get(
            f"{LABEL_PREFIX}.access.app_launcher_visible", "false"
        ).lower() in ["true", "1", "t", "yes"]
        default_access_allowed_idps_label_str = labels.get(f"{LABEL_PREFIX}.access.allowed_idps")
        default_access_auto_redirect_label = labels.get(
            f"{LABEL_PREFIX}.access.auto_redirect_to_identity", "false"
        ).lower() in ["true", "1", "t", "yes"]
        ...
        path_label = labels.get(f"{LABEL_PREFIX}.path")
        ...
        if hostname_label and service_label:
            if is_valid_hostname(hostname_label) and is_valid_service(service_label):
                ...
                logging.warning(
                    f"Ignoring invalid direct label pair for {container_name}: Invalid hostname '{hostname_label}' or service '{service_label}'"
                )
        ...
        while True:
            ...
            path_indexed = labels.get(f"{prefix}.path", path_label)
            ...
            if not service_indexed:
                ...
                if not service_indexed:
                    logging.warning(
                        f"Ignoring indexed hostname {hostname_indexed} for {container_name}: Missing service for index {index} and no default service label."
                    )
                    index += 1
                    continue

            zone_name_indexed = labels.get(f"{prefix}.zonename", labels.get(f"{LABEL_PREFIX}.zonename"))
            no_tls_verify_indexed_value = labels.get(
                f"{prefix}.no_tls_verify", labels.get(f"{LABEL_PREFIX}.no_tls_verify", "false")
            )
            ...
            access_session_duration_indexed = labels.get(
                f"{prefix}.access.session_duration", default_access_session_duration_label
            )
            access_app_launcher_visible_indexed_val = labels.get(
                f"{prefix}.access.app_launcher_visible", str(default_access_app_launcher_visible_label).lower()
            )
            access_app_launcher_visible_indexed = access_app_launcher_visible_indexed_val.lower() in [
                "true",
                "1",
                "t",
                "yes",
            ]
            access_allowed_idps_indexed_str = labels.get(
                f"{prefix}.access.allowed_idps", default_access_allowed_idps_label_str
            )
            access_auto_redirect_indexed_val = labels.get(
                f"{prefix}.access.auto_redirect_to_identity", str(default_access_auto_redirect_label).lower()
            )
            access_auto_redirect_indexed = access_auto_redirect_indexed_val.lower() in ["true", "1", "t", "yes"]
            access_custom_rules_indexed_str = labels.get(
                f"{prefix}.access.custom_rules", default_access_custom_rules_label_str
            )
            ...
            if is_valid_hostname(hostname_indexed) and is_valid_service(service_indexed):
                ...
                logging.warning(
                    f"Ignoring invalid indexed label pair for {container_name} (index {index}): Invalid hostname '{hostname_indexed}' or service '{service_indexed}'"
                )
        ...
        for config_item in hostnames_to_process:
            ...
            if zone_name_from_item:
                ...
                if not target_zone_id:
                    logging.error(
                        f"Failed to find Zone ID for specified name '{zone_name_from_item}' for hostname {hostname}. Skipping."
                    )
                    continue
            else:
            ...
            with state_lock:
                if hostname in managed_rules and managed_rules[hostname].get("source") == "manual":
                    logging.warning(
                        f"Container {container_name} attempting to manage hostname '{hostname}', but it's already a manual entry. Skipping Docker label for this hostname."
                    )
                    continue
                ...
                if existing_rule:
                    ...
                    if existing_rule.get("status") == "pending_deletion":
                        ...
                        path_changed = existing_rule.get("path") != path_from_item

                        if container_changed:
                            logging.info(
                                f"Updating container ID for active rule {hostname}: '{existing_rule.get('container_id')[:12] if existing_rule.get('container_id') else 'N/A'}' -> '{container_id[:12]}'."
                            )
                            existing_rule["container_id"] = container_id
                            state_changed_locally = True
                        if service_changed:
                            logging.info(
                                f"Updating service for active rule {hostname}: '{existing_rule.get('service')}' -> '{service}'."
                            )
                            existing_rule["service"] = service
                            state_changed_locally = True
                            needs_tunnel_config_update = True
                        if no_tls_verify_changed:
                            logging.info(
                                f"Updating noTLSVerify for active rule {hostname}: '{existing_rule.get('no_tls_verify')}' -> '{no_tls_verify_from_item}'."
                            )
                            existing_rule["no_tls_verify"] = no_tls_verify_from_item
                            state_changed_locally = True
                            needs_tunnel_config_update = True
                        if zone_id_changed:
                            logging.warning(
                                f"Zone ID for active rule {hostname} changed ('{existing_rule.get('zone_id')}' -> '{target_zone_id}'). DNS in old zone may be stale."
                            )
                            existing_rule["zone_id"] = target_zone_id
                            state_changed_locally = True
                            needs_tunnel_config_update = True
                ...
                if existing_rule.get("access_policy_ui_override", False):
                    logging.info(
                        f"Access policy for {hostname} (current type: {existing_rule.get('access_policy_type')}) is UI-managed. Skipping label-based Access Policy processing."
                    )
                else:
                    ...
                    if effective_tunnel_id:
                        for config_item_dns in hostnames_to_process:
                            ...
                            if target_zone_id_dns:
                                dns_record_id = create_cloudflare_dns_record(
                                    target_zone_id_dns, hostname_dns, effective_tunnel_id
                                )
                                if dns_record_id:
                                    logging.info(
                                        f"DNS record management in zone {target_zone_id_dns} successful for {hostname_dns}."
                                    )
                                else:
                                    logging.error(
                                        f"CRITICAL: Tunnel config or service for {hostname_dns} might be active but failed to create/verify DNS record in zone {target_zone_id_dns}!"
                                    )
                                    cloudflared_agent_state["last_action_status"] = (
                                        f"Error: Failed creating DNS for {hostname_dns} in zone {target_zone_id_dns}."
                                    )
                            else:
                                logging.error(f"Missing Zone ID - cannot manage DNS record for {hostname_dns}.")
                    else:
                        logging.error(
                            f"Missing effective Tunnel ID - cannot manage DNS records for container {container_name}."
                        )
            else:
                logging.error(
                    f"Failed to update Cloudflare tunnel config after processing start for container {container_name}. DNS records not managed."
                )

    except NotFound:
        logging.warning(
            f"Container {container_name} ({container_id[:12] if container_id else 'Unknown'}) not found during start processing."
        )
...
def schedule_container_stop(container_id):
    ...
    with state_lock:
        ...
        if hostname_to_schedule:
            logging.info(
                f"Container {container_id[:12]} managed hostname '{hostname_to_schedule}'. Marking rule for deletion."
            )
            ...
            if rule.get("status") != "pending_deletion":
                ...
                logging.info(
                    f"Rule for {hostname_to_schedule} scheduled for deletion at {rule['delete_at'].isoformat()}"
                )
                state_changed = True
            else:
...
def docker_event_listener():
    ...
    while not stop_event.is_set() and error_count < max_errors:
        try:
                    ...
                    if action == "start":
                        ...
                        for attempt in range(3):
                            try:
                                ...
                                if container.labels.get(f"{LABEL_PREFIX}.hostname"):
                                    logging.debug(
                                        f"Container {cont_id[:12]} details retrieved on attempt {attempt + 1}."
                                    )
                                    break
                                else:
                                    logging.debug(
                                        f"Container {cont_id[:12]} found but labels might be missing, retrying ({attempt + 1}/3)..."
                                    )
                            except NotFound:
                                logging.debug(
                                    f"Container {cont_id[:12]} not found on attempt {attempt + 1}, retrying..."
                                )
                            except APIError as e:
                                logging.error(
                                    f"Docker API error getting container {cont_id[:12]} on attempt {attempt + 1}: {e}"
                                )
                                break
                            except Exception as e:
                                logging.error(
                                    f"Unexpected error getting container {cont_id[:12]} details: {e}", exc_info=True
                                )
                                break
                            ...
                            if attempt < 2:
                                ...
                                logging.warning(
                                    f"Failed to get container {cont_id[:12]} details after multiple attempts."
                                )
                        ...
                        if container:
                            try:
                                ...
                                logging.error(
                                    f"Unexpected error processing start event for {cont_id[:12]} after retrieval: {e}",
                                    exc_info=True,
                                )
                        ...
                        try:
                            ...
                            logging.error(
                                f"Unexpected error processing stop/die/destroy event for {cont_id[:12]}: {e}",
                                exc_info=True,
                            )
            ...
            error_count += 1
            logging.error(
                f"Unexpected error in Docker event listener: {e}. Reconnecting ({error_count}/{max_errors})...",
                exc_info=True,
            )
            stop_event.wait(min(30, 5 * error_count))
        ...
        try:
                ...
                for hostname, details in list(managed_rules.items()):
                    if details.get("status") == "pending_deletion" and details.get("source", "docker") == "docker":
                        ...
                        if isinstance(delete_at, datetime):
                            ...
                            logging.warning(
                                f"Rule {hostname} pending delete but has invalid delete_at timestamp: {delete_at}. Marking for immediate deletion processing."
                            )
                            is_expired = True
                        ...
                        if is_expired:
                            ...
                            if not zone_id_for_delete:
                                logging.error(
                                    f"Cannot schedule DNS deletion for expired rule {hostname}: Zone ID is missing in state and no default CF_ZONE_ID is set. Rule will be removed from state only."
                                )
                            ...
                            logging.info(
                                f"Rule for {hostname} (Zone: {zone_id_for_delete}, Access App: {access_app_id_for_delete}) expired. Scheduling for full deletion."
                            )
                    elif details.get("source") == "manual" and details.get("status") == "pending_deletion":
                        logging.warning(
                            f"Manual rule {hostname} found in 'pending_deletion' state. This should not happen. Resetting to 'active'."
                        )
            ...
            if rules_to_process_for_deletion:
                ...
                for hostname, delete_info in rules_to_process_for_deletion.items():
                    ...
                    if zone_id and effective_tunnel_id:
                        ...
                        if delete_cloudflare_dns_record(zone_id, hostname, effective_tunnel_id):
                            ...
                            logging.error(
                                f"Failed to delete DNS record for {hostname} in zone {zone_id}. It may remain stale."
                            )
                    elif not zone_id:
                    ...
                    if access_app_id:
                        logging.info(
                            f"Attempting Access Application deletion for expired rule: {hostname}, App ID: {access_app_id}"
                        )
                        if delete_cloudflare_access_application(access_app_id):
                            access_app_deleted_successfully = True
                        else:
                            logging.error(
                                f"Failed to delete Access Application {access_app_id} for {hostname}. It may remain orphaned."
                            )
                    else:
                        access_app_deleted_successfully = True
                ...
                if hostnames_fully_cleaned_for_state_removal:
                    logging.info(
                        f"Attempting Cloudflare tunnel config update after processing deletions for: {hostnames_fully_cleaned_for_state_removal}"
                    )
                    if update_cloudflare_config() or USE_EXTERNAL_CLOUDFLARED:
                        logging.info(
                            f"Cloudflare tunnel config update successful (or skipped for external). Removing rules from local state: {hostnames_fully_cleaned_for_state_removal}"
                        )
                        with state_lock:
                            deleted_count = 0
                            for hostname in hostnames_fully_cleaned_for_state_removal:
                                if (
                                    hostname in managed_rules
                                    and managed_rules[hostname].get("status") == "pending_deletion"
                                    and managed_rules[hostname].get("source", "docker") == "docker"
                                ):
                                    ...
                                    logging.warning(
                                        f"Rule {hostname} was scheduled for removal but not found or not pending/docker when removing from state."
                                    )
                        ...
                        logging.error(
                            "Failed to update Cloudflare tunnel config during rule cleanup. Rules remain in local state but their cloud resources might have been deleted. Will retry on next cycle or reconciliation."
                        )
                else:
...
def reconcile_state():
    ...
    reconcile_thread = threading.Thread(target=_run_reconciliation, name="ReconciliationThread", daemon=True)
...
def _run_reconciliation():
    ...
    app.reconciliation_info = {
        ...
        "total_items": 0,
        "processed_items": 0,
        "start_time": reconciliation_start,
    ...
    try:
        ...
        try:
            ...
            for i in range(0, container_count, batch_size):
                ...
                app.reconciliation_info["progress"] = (
                    min(100, int((processed_container_count / container_count) * 100)) if container_count > 0 else 0
                )
                app.reconciliation_info["processed_items"] = processed_container_count
                app.reconciliation_info["status"] = (
                    f"Scanning containers: batch {i // batch_size + 1}/{(container_count + batch_size - 1) // batch_size}"
                )
                ...
                for c in batch:
                    ...
                    try:
                        ...
                        default_app_launcher_visible = labels.get(
                            f"{LABEL_PREFIX}.access.app_launcher_visible", "false"
                        ).lower() in ["true", "1", "t", "yes"]
                        default_allowed_idps_str = labels.get(f"{LABEL_PREFIX}.access.allowed_idps")
                        default_auto_redirect = labels.get(
                            f"{LABEL_PREFIX}.access.auto_redirect_to_identity", "false"
                        ).lower() in ["true", "1", "t", "yes"]
                        ...
                        path_direct = labels.get(f"{LABEL_PREFIX}.path")
                        ...
                        while time.time() - container_scan_item_start_time < 5:
                            ...
                            path_idx = labels.get(f"{pfx}.path", path_direct)
                            zn_idx = labels.get(f"{pfx}.zonename", zn)
                            ntv_idx = labels.get(f"{pfx}.no_tls_verify", str(ntv).lower()).lower() in [
                                "true",
                                "1",
                                "t",
                                "yes",
                            ...
                            acc_vis_idx = labels.get(
                                f"{pfx}.access.app_launcher_visible", str(default_app_launcher_visible).lower()
                            ).lower() in ["true", "1", "t", "yes"]
                            acc_idps_idx = labels.get(f"{pfx}.access.allowed_idps", default_allowed_idps_str)
                            acc_redir_idx = labels.get(
                                f"{pfx}.access.auto_redirect_to_identity", str(default_auto_redirect).lower()
                            ).lower() in ["true", "1", "t", "yes"]
                            ...
                            if is_valid_hostname(h_idx) and is_valid_service(s_idx):
                                hostname_configs_for_container.append({
                                    ...
                                    "access_policy_type": acc_pol_idx,
                                    "access_app_name": acc_name_idx,
                                    "access_session_duration": acc_sess_idx,
                                    "access_app_launcher_visible": acc_vis_idx,
                                    "access_allowed_idps_str": acc_idps_idx,
                                    "access_auto_redirect": acc_redir_idx,
                        ...
                        for config_item in hostname_configs_for_container:
                            ...
                            if hostname_val in running_labeled_hostnames_details:
                                logging.warning(
                                    f"[Reconcile] Duplicate hostname '{hostname_val}' found from labels. Using latest encountered: {container_name_val}."
                                )
                        ...
                        logging.error(
                            f"[Reconcile] Error processing container {c.id[:12] if c and c.id else 'N/A'}: {e_cont}"
                        )
                        continue
            ...
            logging.info(
                f"[Reconcile] Found {len(running_labeled_hostnames_details)} running hostnames with DockFlare labels."
            )
        ...
        try:
            ...
            if not state_lock_acquired:
                ...
                for hostname, desired_details in running_labeled_hostnames_details.items():
                    ...
                    app.reconciliation_info["progress"] = (
                        min(100, int((processed_reconcile_items / app.reconciliation_info["total_items"]) * 100))
                        if app.reconciliation_info["total_items"] > 0
                        else 0
                    )
                    ...
                    if existing_rule_for_hostname and existing_rule_for_hostname.get("source") == "manual":
                        logging.debug(
                            f"[Reconcile] Hostname {hostname} is manually managed. Skipping update from Docker labels for container {desired_details.get('container_name', 'N/A')}."
                        )

                        continue

                    target_zone_id = (
                        get_zone_id_from_name(desired_details["zone_name"])
                        if desired_details["zone_name"]
                        else CF_ZONE_ID
                    )
                    if not target_zone_id:
                    ...
                    if not current_rule:
                        ...
                        if current_rule.get("status") == "pending_deletion":
                            ...
                            current_rule["delete_at"] = None
                            state_changed_locally = True

                        service_changed_in_reconcile = current_rule.get("service") != desired_details["service"]
                        path_changed_in_reconcile = current_rule.get("path") != desired_details.get("path")
                        no_tls_verify_changed_in_reconcile = (
                            current_rule.get("no_tls_verify") != desired_details["no_tls_verify"]
                        )
                        zone_id_changed_in_reconcile = current_rule.get("zone_id") != target_zone_id
                        container_id_changed_in_reconcile = (
                            current_rule.get("container_id") != desired_details["container_id"]
                        )

                        if (
                            service_changed_in_reconcile
                            or no_tls_verify_changed_in_reconcile
                            or zone_id_changed_in_reconcile
                            or path_changed_in_reconcile
                        ):
                            needs_tunnel_config_update = True

                        current_rule["service"] = desired_details["service"]
                        current_rule["path"] = desired_details.get("path")
                        ...
                        if (
                            ...
                            or no_tls_verify_changed_in_reconcile
                            or zone_id_changed_in_reconcile
                            or container_id_changed_in_reconcile
                            or path_changed_in_reconcile
                        ):
                            state_changed_locally = True
                    ...
                    if current_rule.get("access_policy_ui_override", False):
                        logging.info(
                            f"[Reconcile] Access policy for {hostname} (current type: {current_rule.get('access_policy_type')}) is UI-managed. Skipping label-based Access Policy processing during reconciliation."
                        )
                    else:
                        logging.debug(
                            f"[Reconcile] Processing Access Policy from labels for {hostname} (not UI-managed) during reconciliation."
                        )
                        if _handle_access_policy_from_labels(desired_details, current_rule):
                            state_changed_locally = True

                hostnames_in_state_but_not_running = list(
                    current_managed_hostnames_in_state - set(running_labeled_hostnames_details.keys())
                )
                ...
                for hostname_to_check in hostnames_in_state_but_not_running:
                    ...
                    if rule and rule.get("status") == "active" and rule.get("source", "docker") == "docker":
                        logging.info(
                            f"[Reconcile] Docker-managed rule for {hostname_to_check} is active but container/labels missing. Marking for deletion."
                        )
                        rule["status"] = "pending_deletion"
        ...
        app.reconciliation_info["progress"] = 100
        app.reconciliation_info["status"] = (
            app.reconciliation_info.get("status", "Reconciliation finished.") + " (Final)"
        )
        ...
        watchdog.cancel()
        logging.info(
            f"[Reconcile Thread] Reconciliation complete. Duration: {duration:.2f} seconds. Status: {app.reconciliation_info['status']}"
        )
...
def update_cloudflared_container_status():
        ...
        if current_status != "docker_unavailable":
            ...
            try:
                ...
                if current_status != new_status:
                    logging.info(
                        f"Agent container '{CLOUDFLARED_CONTAINER_NAME}' status updated post-reconnect: {current_status} -> {new_status}"
                    )
                ...
                if current_status != "docker_unavailable":
                    logging.info(
                        f"Agent container '{CLOUDFLARED_CONTAINER_NAME}' status changed: {current_status} -> docker_unavailable"
                    )
    ...
    if container:
        try:
            ...
            if current_status != new_status:
                logging.info(
                    f"Agent container '{CLOUDFLARED_CONTAINER_NAME}' status changed: {current_status} -> {new_status}"
                )
                cloudflared_agent_state["container_status"] = new_status
...
def ensure_docker_network_exists(network_name):
    ...
    try:
        ...
        try:
            ...
            cloudflared_agent_state["last_action_status"] = f"Error: Unexpected create net: {e}"
            return False
    except APIError as e:
        ...
        if container:
            try:
                ...
                if network_mode == "host" or network_mode == CLOUDFLARED_NETWORK_NAME:
                    logging.warning(
                        f"Existing container '{CLOUDFLARED_CONTAINER_NAME}' is in an unexpected network mode ('{network_mode}'). Needs recreation."
                    )
                    needs_recreate = True
                elif CLOUDFLARED_NETWORK_NAME not in current_networks:
                    logging.warning(
                        f"Existing container '{CLOUDFLARED_CONTAINER_NAME}' is not attached to the required network '{CLOUDFLARED_NETWORK_NAME}'. Needs recreation."
                    )
                    needs_recreate = True

                if needs_recreate:
                    logging.info(
                        f"Removing misconfigured/stopped container '{CLOUDFLARED_CONTAINER_NAME}' before creating a new one..."
                    )
                    try:
                        ...
                        logging.error(
                            f"Failed to remove misconfigured container '{CLOUDFLARED_CONTAINER_NAME}': {rm_err}. Cannot proceed."
                        )
                ...
                logging.warning(
                    f"Error checking existing container '{CLOUDFLARED_CONTAINER_NAME}': {e}. Assuming creation is needed."
                )
                container = None
            except requests.exceptions.ConnectionError as e:
                logging.error(f"Docker connection error checking existing container: {e}")
                cloudflared_agent_state["last_action_status"] = "Error: Docker connect check agent."
                return False
        ...
        if not container and not success_flag:
            ...
            try:
                try:
                    ...
                    logging.warning(
                        f"Could not pull image {CLOUDFLARED_IMAGE}: {img_err}. Container run will attempt using local image if available."
                    )
                except requests.exceptions.ConnectionError as e:
                    logging.error(f"Docker connection failed during image pull: {e}")
                    cloudflared_agent_state["last_action_status"] = "Error: Docker connect pull image."
                    return False
                ...
                if "is already in use" in str(create_err):
                    logging.error(
                        f"Container name '{CLOUDFLARED_CONTAINER_NAME}' is already in use by another container."
                    )
                ...
                cloudflared_agent_state["last_action_status"] = "Error: Docker connect run agent."
                success_flag = False

    except APIError as e:
        msg = f"Docker API error during start sequence: {e}"
        logging.error(msg, exc_info=True)
        cloudflared_agent_state["last_action_status"] = f"Error: {msg}"
        success_flag = False
    except requests.exceptions.ConnectionError as e:
        msg = f"Docker connection error during start sequence: {e}"
        logging.error(msg)
        cloudflared_agent_state["last_action_status"] = f"Error: {msg}"
        success_flag = False
    except Exception as e:
...
def stop_cloudflared_container():
    ...
    try:
        ...
        container = get_cloudflared_container()
        if not container:
            msg = f"Container '{CLOUDFLARED_CONTAINER_NAME}' not found (already stopped or removed?)."
            logging.warning(msg)
            cloudflared_agent_state["last_action_status"] = msg
        ...
        msg = f"Docker API error stopping container '{CLOUDFLARED_CONTAINER_NAME}': {e}"
        logging.error(msg, exc_info=True)
        cloudflared_agent_state["last_action_status"] = f"Error: {msg}"
        success_flag = False
    except requests.exceptions.ConnectionError as e:
        ...
        msg = f"Unexpected error stopping container '{CLOUDFLARED_CONTAINER_NAME}': {e}"
        logging.error(msg, exc_info=True)
        cloudflared_agent_state["last_action_status"] = f"Error: {msg}"
        success_flag = False
    finally:
    ...
    with state_lock:
        ...
        for hostname, rule_details in managed_rules.items():
            if rule_details.get("status") == "active":
                ...
                path_value = rule_details.get("path")
                ...
                if service:
                    ...
                    if path_value and path_value.strip():
                        processed_path = path_value.strip()
                        if not processed_path.startswith("/"):
                            logging.warning(
                                f"Path '{processed_path}' for hostname '{hostname}' does not start with '/'. Prepending '/'."
                            )
        ...
        has_catch_all = any(
            r.get("service") == catch_all_rule_template["service"] and not r.get("hostname")
        ...
        current_api_comparable_set = {
            json.dumps(rule_to_comparable_dict(r), sort_keys=True) for r in current_api_ingress_rules
        }
        final_put_comparable_set = {
            json.dumps(rule_to_comparable_dict(r), sort_keys=True) for r in final_ingress_rules_to_put
        }
...
def get_all_account_cloudflare_tunnels():
    ...
    try:
        ...
        if isinstance(tunnels, list):
            ...
            filtered_tunnels = [tunnel for tunnel in tunnels if tunnel.get("status") in desired_statuses]

            logging.info(
                f"Returning {len(filtered_tunnels)} tunnels after client-side status check for relevant statuses."
            )
            filtered_tunnels.sort(key=lambda t: t.get("name", "").lower())
            return filtered_tunnels
        else:
            logging.error(
                f"Unexpected data format for account tunnels list: {type(tunnels)}. Expected a list. Response: {response_data}"
            )
            return []
    except requests.exceptions.RequestException as e:
        ...
        if hasattr(e, "response") and e.response is not None:
            if e.response.status_code == 403:
                logging.error(
                    "Permission denied (403) listing account tunnels. Ensure API token has 'Account:Cloudflare Tunnel:Read' permission for the account."
                )
...
def get_dns_records_for_tunnel(zone_id, tunnel_id):
    ...
    logging.info(
        f"Fetching DNS records for tunnel {tunnel_id} in zone {zone_name_for_display} ({zone_id}) with content {expected_cname_content}"
    )
    ...
    try:
        ...
        if isinstance(dns_records_from_api, list):
            ...
            logging.error(
                f"Unexpected data format for DNS records list in zone {zone_name_for_display}: {type(dns_records_from_api)}"
            )
            return []
    except requests.exceptions.RequestException as e:
        ...
        logging.error(
            f"Unexpected error fetching DNS records for tunnel {tunnel_id} in zone {zone_name_for_display}: {e}",
            exc_info=True,
        )
        return []
...
def ui_update_access_policy(hostname):
    ...
    logging.info(
        f"UI Request: Update Access Policy for '{hostname}' to type '{new_policy_type}' with email '{auth_email if auth_email else 'N/A'}'."
    )
    ...
    with state_lock:
        ...
        desired_session_duration = request.form.get(
            "session_duration", current_rule.get("access_session_duration", "24h")
        )
        desired_app_launcher_visible = request.form.get(
            "app_launcher_visible", str(current_rule.get("access_app_launcher_visible", False))
        ).lower() in ["true", "1", "t", "yes"]
        desired_allowed_idps_str = request.form.get("allowed_idps", current_rule.get("access_allowed_idps_str"))
        desired_auto_redirect = request.form.get(
            "auto_redirect", str(current_rule.get("access_auto_redirect", False))
        ).lower() in ["true", "1", "t", "yes"]
        ...
        if new_policy_type == "none" or new_policy_type == "public_no_policy":
            ...
            if not auth_email:
                cloudflared_agent_state["last_action_status"] = (
                    f"Error: Email address required for 'authenticate_email' policy for {hostname}."
                )
                return redirect(url_for("status_page"))
            cf_access_policies = [
                {
                    "name": f"UI Allow Email {auth_email}",
                    "decision": "allow",
                    "include": [{"email": {"email": auth_email}}],
                },
        ...
        if new_policy_type in ["bypass", "authenticate_email"]:
            if not cf_access_policies:
                logging.error(
                    f"UI: No policies defined for {hostname} with type {new_policy_type} but expected. Aborting."
                )
            ...
            allowed_idps_list_for_app = (
                [idp.strip() for idp in desired_allowed_idps_str.split(",") if idp.strip()]
            ...
            if current_access_app_id:
                if (
                    current_rule.get("access_policy_type") != final_policy_type_for_state
                    or current_rule.get("access_app_config_hash") != new_config_hash
                ):
                    logging.info(
                        f"UI: Attempting to update Access App. ID: {current_access_app_id}, Target Name: {desired_app_name}, Target Policy: {final_policy_type_for_state}"
                    )
                    updated_app = update_cloudflare_access_application(
                        current_access_app_id,
                        hostname,
                    ...
                    if updated_app:
                        ...
                        action_status_message = (
                            f"Success: Access Policy for {hostname} updated to {final_policy_type_for_state}."
                        )
                    else:
                ...
                logging.info(
                    f"UI: Attempting to create Access App. Target Name: {desired_app_name}, Target Policy: {final_policy_type_for_state}"
                )
                created_app = create_cloudflare_access_application(
                    hostname,
                    desired_app_name,
                ...
                if created_app and created_app.get("id"):
                    ...
                    operation_successful = True
                    action_status_message = (
                        f"Success: Access Policy for {hostname} created as {final_policy_type_for_state}."
                    )
                else:
        ...
        if operation_successful:
            ...
            logging.warning(
                f"UI operation for {hostname} failed or no effective change made that requires API action. Override flag status based on prior state or this action's success."
            )
...
def tunnel_dns_records(tunnel_id):
    ...
    logging.info(
        f"Found {len(all_found_dns_records)} DNS records for tunnel {tunnel_id} across {len(zone_ids_to_scan)} zones."
    )
    ...
    with state_lock:
        for hostname, rule in managed_rules.items():
            ...
            if rule_copy.get("delete_at") and isinstance(rule_copy["delete_at"], datetime):
                try:
                    rule_copy["delete_at"] = (
                        rule_copy["delete_at"].replace(tzinfo=timezone.utc)
                        if rule_copy["delete_at"].tzinfo is None
                        else rule_copy["delete_at"].astimezone(timezone.utc)
                    )
                except Exception as date_parse_err:
                    logging.warning(
                        f"Error preparing delete_at ('{rule_copy['delete_at']}') for template: {date_parse_err}"
                    )
        ...
        if CF_ZONE_ID and docker_client:
            ...
            if cached_zone_name:
                ...
                logging.debug(
                    f"Using cached zone name '{relevant_zone_name_for_tld_policy}' for CF_ZONE_ID '{CF_ZONE_ID}' for TLD check."
                )
                ...
                if CF_ZONE_ID == resolved_id_for_tunnel_zone:
                    ...
                    logging.debug(
                        f"Derived zone name '{relevant_zone_name_for_tld_policy}' from TUNNEL_NAME and it matches CF_ZONE_ID."
                    )
            ...
            if relevant_zone_name_for_tld_policy:
                ...
                logging.info(
                    "Relevant zone name for TLD policy check could not be determined (CF_ZONE_ID might be set, but its name is not cached or derivable). Skipping TLD policy UI elements."
                )
        else:
            if not CF_ZONE_ID:
...
def revert_access_policy_to_labels(hostname):
    ...
    with state_lock:
        ...
        if not current_rule.get("access_policy_ui_override", False):
            action_status_message = (
                f"Info: Access Policy for '{hostname}' is already managed by labels. No action taken."
            )
            logging.info(action_status_message)
            cloudflared_agent_state["last_action_status"] = action_status_message
            return redirect(url_for("status_page"))
    ...
    if app_id_to_delete_if_any:
        logging.info(
            f"Reverting policy for '{hostname}': Deleting previously UI-managed Access App ID '{app_id_to_delete_if_any}'."
        )
...
def force_delete_rule(hostname):
    ...
    with state_lock:
        ...
        if rule_details:
            ...
            logging.info(
                f"Found rule for {hostname} with Zone ID: {zone_id_for_delete} and Access App ID: {access_app_id_for_delete}"
            )
        else:
            logging.warning(
                f"Rule {hostname} not found in state during force delete. Attempting DNS delete in default zone ID ({CF_ZONE_ID}) if available. Access App cannot be determined."
            )
    ...
    if zone_id_for_delete and effective_tunnel_id:
        logging.info(
            f"Attempting immediate DNS record deletion for force-deleted rule: {hostname} in zone {zone_id_for_delete} using tunnel {effective_tunnel_id}"
        )
    ...
    if access_app_id_for_delete:
        logging.info(
            f"Attempting immediate Access Application deletion for force-deleted rule: {hostname}, App ID: {access_app_id_for_delete}"
        )
    ...
    if rule_removed_from_state:
        if not USE_EXTERNAL_CLOUDFLARED:
            ...
            if update_cloudflare_config():
                ...
                logging.error(
                    f"CRITICAL: State updated after force delete of {hostname}, but subsequent tunnel config update FAILED!"
                )
                status_msg_parts.append("Tunnel config update FAILED!")
                cloudflared_agent_state["last_action_status"] = (
                    f"Error: Removed {hostname} (DNS: {dns_delete_success}, Access: {access_app_delete_success}), but FAILED tunnel config update! Reconciliation needed."
                )
        else:
...
def ui_add_manual_rule():
    if not docker_client:
        cloudflared_agent_state["last_action_status"] = (
            "Error: System not ready to add manual rule. Docker client unavailable."
        )
        return redirect(url_for("status_page"))
    if not tunnel_state.get("id"):
        cloudflared_agent_state["last_action_status"] = (
            "Error: System not ready to add manual rule. Tunnel not initialized."
        )
        return redirect(url_for("status_page"))
    ...
    try:
        ...
        logging.info(
            f"Manual rule service input '{service_input}' processed to '{processed_service_for_cf}' for Cloudflare config."
        )
    ...
    if not is_valid_service(processed_service_for_cf):
        cloudflared_agent_state["last_action_status"] = (
            f"Error: Processed service URL '{processed_service_for_cf}' (from input '{service_input}') is invalid for Cloudflare."
        )
        return redirect(url_for("status_page"))
    ...
    if zone_name:
        ...
        cloudflared_agent_state["last_action_status"] = (
            "Error: Zone Name required for manual rule as CF_ZONE_ID is not set."
        )
        return redirect(url_for("status_page"))
    ...
    with state_lock:
        ...
        if existing_rule_details and existing_rule_details.get("source", "docker") == "docker":
            cloudflared_agent_state["last_action_status"] = (
                f"Error: Hostname {hostname} is already managed by Docker labels. Manual rule not added/updated."
            )
            return redirect(url_for("status_page"))
        ...
        managed_rules[hostname] = {
            ...
            "access_app_config_hash": existing_rule_details.get("access_app_config_hash")
            if existing_rule_details
            else None,
            "access_policy_ui_override": existing_rule_details.get("access_policy_ui_override", False)
            if existing_rule_details
            else False,
            "source": "manual",
        }
    ...
    if not effective_tunnel_id:
        cloudflared_agent_state["last_action_status"] = (
            f"Error: Cannot setup DNS/Tunnel for {hostname}, effective tunnel ID missing."
        )
        return redirect(url_for("status_page"))
    ...
    if update_cloudflare_config():
        ...
        cloudflared_agent_state["last_action_status"] = (
            f"Success: Manual rule for {hostname} (service: {processed_service_for_cf}) added/updated."
        )
    else:
        cloudflared_agent_state["last_action_status"] = (
            f"Error: Failed to update Cloudflare config for manual rule {hostname}. DNS record might also be affected."
        )
...
def ui_delete_manual_rule(hostname):
    if not docker_client:
        cloudflared_agent_state["last_action_status"] = (
            "Error: System not ready to delete manual rule. Docker client unavailable."
        )
        return redirect(url_for("status_page"))
    if not tunnel_state.get("id"):
        cloudflared_agent_state["last_action_status"] = (
            "Error: System not ready to delete manual rule. Tunnel not initialized."
        )
        return redirect(url_for("status_page"))
    ...
    with state_lock:
        ...
        if rule_details and rule_details.get("source") == "manual":
            ...
            cloudflared_agent_state["last_action_status"] = (
                f"Error: Rule for {hostname} is not a manual rule. Cannot delete via this action."
            )
            return redirect(url_for("status_page"))
        else:
    ...
    if rule_existed_as_manual:
        ...
        if not effective_tunnel_id:
            cloudflared_agent_state["last_action_status"] = (
                f"Warning: Rule {hostname} deleted from state, but DNS/Tunnel resources cannot be cleaned (missing tunnel ID)."
            )
            return redirect(url_for("status_page"))
        ...
        if update_cloudflare_config():
            cloudflared_agent_state["last_action_status"] = (
                f"Success: Manual rule {hostname} and associated resources deleted."
            )
        else:
            cloudflared_agent_state["last_action_status"] = (
                f"Warning: Manual rule {hostname} deleted from state & DNS/Access attempted, but Cloudflare tunnel config update failed."
            )
...
def cloudflare_ping():
    ...
    try:
        ...
        return jsonify({"error": str(e), "status": "error", "timestamp": int(time.time())}), 500
...
if __name__ == "__main__":
    ...
    def initialization_process():
        ...
        if USE_EXTERNAL_CLOUDFLARED and tunnel_state.get("id"):
            ...
            try:
                ...
                try:
                    ...
                    for i in range(0, container_count, batch_size):
                        ...
                        app.reconciliation_info["status"] = (
                            f"Initial scan: batch {i // batch_size + 1}/{(container_count + batch_size - 1) // batch_size}"
                        )
                        app.reconciliation_info["total_items"] = container_count
                        processed += len(batch)
            ...
            logging.warning(
                "Tunnel not fully initialized. Skipping reconciliation, agent start, and event/cleanup tasks."
            )
            if not tunnel_state.get("error"):
    ...
    if not docker_client:
        ...
        agent_status_thread = threading.Thread(
            target=periodic_agent_status_updater, name="AgentStatusUpdater", daemon=True
        )
        agent_status_thread.start()

        init_thread = threading.Thread(target=initialization_process, name="InitializationProcess", daemon=True)
        init_thread.start()
    ...
    try:
        ...
        flask_thread = threading.Thread(
            target=serve, args=(app,), kwargs={"host": "0.0.0.0", "port": 5000}, daemon=True, name="FlaskWaitressServer"
        )
        flask_thread.start()
        ...
        logging.warning(
            "Waitress not found. Using Flask development server (not recommended for production). Install using: pip install waitress"
        )
