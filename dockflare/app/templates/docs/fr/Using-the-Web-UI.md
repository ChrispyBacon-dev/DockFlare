...
L'interface utilisateur Web DockFlare est un outil puissant pour gérer, surveiller et configurer vos services. Il fournit une interface conviviale pour les tâches qui vont au-delà de la simple configuration des étiquettes Docker.

## Le tableau de bord (page principale)

La première page que vous voyez après la connexion est le tableau de bord principal. Il s'agit de votre plateforme centrale pour visualiser l'état de tous vos services gérés.
...
* **Tableau des règles d'entrée gérées :** Ce tableau répertorie toutes les règles d'entrée gérées par DockFlare, qu'elles proviennent d'un conteneur Docker ou qu'elles aient été créées manuellement.
    ...
    * **Source :** Indique si la règle provient de `Docker` ou a été créée `Manually` dans l'interface utilisateur.
    * **Statut :** Indique si la règle est `active`, `pending_deletion` ou si elle a un `UI Override`.
    * **Accès :** Affiche le groupe d'accès et le badge de mode appliqués. Attendez-vous à voir des étiquettes `Public` ou `Authenticated`, des noms de groupes en cascade et des liens rapides vers le tableau de bord Cloudflare lors de la synchronisation des politiques réutilisables.
    * **Gérer la règle :** Ce bouton vous permet de modifier n'importe quelle règle.
* **Journaux en temps réel :** Sous le tableau, vous trouverez une visionneuse de journaux en temps réel qui diffuse les journaux du backend DockFlare, ce qui est inestimable pour le débogage.
...
* **Modifier n'importe quelle règle :** Le bouton « Gérer la règle » à côté de chaque règle ouvre une fenêtre modale dans laquelle vous pouvez modifier sa configuration. C'est ainsi que vous pouvez appliquer un remplacement d'interface utilisateur à une règle créée à l'origine à partir d'étiquettes Docker.
* **Revenir aux étiquettes :** Si une règle de Docker a un remplacement d'interface utilisateur, un bouton "Revenir aux étiquettes" apparaîtra, vous permettant d'ignorer vos modifications manuelles et de laisser la règle être à nouveau contrôlée par ses étiquettes Docker.
...
* **Filet de sécurité :** Même si vous oubliez d'ajouter une stratégie à un service spécifique, la stratégie générique au niveau de la zone la détectera.
