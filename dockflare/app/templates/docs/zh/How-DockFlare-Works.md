...
3. **调用 Cloudflare API**：根据这些标签，DockFlare 会在 Cloudflare 中配置所需资源：
   * **Cloudflare Tunnel**：在指定的 Cloudflare Tunnel 中添加一条 ingress 规则，将公网 hostname 指向容器的内部网络地址，例如 `http://my-app:8080`。
   * **DNS 管理**：在 Cloudflare DNS 区域中创建 CNAME 记录，将您期望的公网 hostname，例如 `my-app.example.com`，指向该 Cloudflare Tunnel。
   * **Access 策略**：如果您定义了访问控制标签，DockFlare 会创建或更新可复用的 Cloudflare Access 策略，通过 Zero Trust 规则保护您的服务，例如要求使用身份提供商登录，或者应用公共 `bypass`。

4. **自动清理**：当受管容器停止或被删除时，DockFlare 会自动执行清理流程。它会从 Cloudflare Tunnel 中删除对应的 ingress 规则；如果没有其他服务继续使用该 hostname，还会删除相应的 DNS 记录和 Access 应用。这样可以避免残留配置，让 Cloudflare 侧保持整洁。

## 组件概览

| 组件 | 职责 |
| --- | --- |
| DockFlare Master | 托管 UI 和 API、监控 Docker 事件，并编排 Cloudflare 隧道、DNS 和 Access 策略。它以无 root 方式运行，并且只通过 socket proxy 与 Docker 通信。 |
| Docker Socket Proxy | `tecnativa/docker-socket-proxy` sidecar，向 Master 暴露最小范围的 Docker API（`containers`、`events` 等），避免 Master 直接挂载原始 Docker 套接字。 |
| Redis | 用于缓存、队列、日志流以及代理 heartbeat/回传通道。它运行在私有的 `dockflare-internal` 网络中。 |
| DockFlare Agents（可选） | 远程代理进程，在其他主机上复现 Master 的行为，将 Docker 事件流式传回，并管理自己的 `cloudflared`。 |
| `cloudflared` | 维护 Master 或各个代理到 Cloudflare 的隧道连接。 |
...
2. **访问组（抽象层）**：为了避免在多个服务中重复定义复杂的访问策略，您可以在 Web UI 中创建可复用的**访问组**。这些模板会打包一组访问规则，例如允许公司邮箱登录或允许某些国家访问，并同步到命名的 Cloudflare Access 可复用策略。弹窗中的 Public vs Authenticated 开关决定 DockFlare 最终生成 `bypass` 还是 `allow`。之后，您只需要通过单个标签，例如 `dockflare.access.group=my-policy-group`，就能把整套策略应用到容器上。

3. **Web UI 覆盖（控制层）**：Web UI 提供最高级别的控制。从仪表板中，您可以：
   * **覆盖**任意服务的访问策略，无论它原本是由标签还是访问组定义的。这些覆盖是持久的，不会因容器重启而丢失。
   * **为非 Docker 服务创建手动 ingress 规则**，例如为局域网中另一台机器上的服务创建入口。
   * **恢复**服务配置到 Docker 标签定义的状态，并丢弃 UI 中做出的覆盖。

这种分层模型让您可以对大多数服务做到“配置好就不用再管”，同时仍保留通过 Web UI 处理例外情况和复杂场景的能力。
