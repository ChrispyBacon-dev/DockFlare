...
> **📌 重要提示：** 本指南用于配置 **DockFlare Web UI 身份验证**（登录 DockFlare 本身）。如果您想为 **Cloudflare 访问策略** 配置 OAuth/OIDC 以保护您的服务，请参阅 [身份提供商](Identity-Providers.md)。

DockFlare 支持通过 OpenID Connect (OIDC) 标准将用户身份验证委托给外部身份提供商。这样可以为 DockFlare Web 界面启用单点登录 (SSO)，并与 Google、Authentik、Okta 等 IdP 集成。
...
在使用 OAuth 身份验证时，你通常会希望用访问策略保护 DockFlare 主界面，同时确保 OAuth 回调路径能够正常工作。如果你的 DockFlare 实例配置了 IP 限制或其他访问控制，这一点尤其重要。
