...
您可以通过几种方式验证这一点：
* **DockFlare Web UI**：`nginx.example.com` 服务将出现在仪表板上。
* **Cloudflare Dashboard**：您将在 DNS 设置中看到新的 CNAME 记录，并在隧道配置中看到新的入口规则。
...
|文件 |描述 |
| ---| ---|
| `dockflare_config.dat` |加密配置负载（Cloudflare 凭证、UI 密码哈希、Tunnel 默认值、Master API key 等）。 |
| `dockflare.key` |用于解密 `dockflare_config.dat` 与其他加密负载的 Fernet key。请与归档一起保存。 |
| `agent_keys.dat` |Agent API key、元数据与吊销状态的加密注册表。 |
| `state.json` |运行时状态的明文 JSON 快照（托管规则、Agents、访问组）。包含该文件是为了便于运维人员检查或在需要时迁移部分状态。 |
| `manifest.json` |存档中每个文件的校验和和版本控制信息。 |

备份是自包含的：通过向导/上传端点恢复时，会将每个文件写入 `/app/data/`，并立即安排容器重启，以便在启动时重新加载加密配置。

### 恢复和兼容性说明

- **向导和 Settings UI**：上传 `.zip`，DockFlare 将导入它，重新加载状态，然后退出。Docker 会自动重启容器，因此您无需手动干预即可返回运行状态。
