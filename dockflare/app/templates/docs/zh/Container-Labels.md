...
> **提示：** 从 DockFlare v3.0 开始，您可以针对大多数工作负载跳过 `dockflare.zonename`。Master 会通过匹配主机名后缀来检测正确的 Cloudflare 区域，并且仅在找不到匹配项时回退到配置的默认区域。当您需要将记录显式创建到其他区域时，再提供该标签。
...
快速创建选项已从 UI 中删除，以鼓励这种最佳实践工作流程。

#### 区域默认策略标签

`dockflare.access.policy=default_tld` 标签仍然有效，并将继承您区域的 `*.domain.com` 通配符策略的保护。如果不存在区域策略，则该服务将是公共的（无访问应用程序）。

**建议：** 在 UI 中为所有域创建区域默认策略，以提高安全性。

|标签|描述 |示例|
| :--- | :--- | :--- |
| `dockflare.access.group` |应用到此服务的单个预配置访问组的 ID。该 ID 可以在 DockFlare UI 的“访问策略”页面上找到。 | `dockflare.access.group=internal-tools-policy` |
