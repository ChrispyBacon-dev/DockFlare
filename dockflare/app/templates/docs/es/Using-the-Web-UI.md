...
La Web UI de DockFlare es una herramienta potente para administrar, monitorear y configurar sus servicios. Proporciona una interfaz fácil de usar para tareas que van más allá de la simple configuración mediante etiquetas de Docker.

## El panel (página principal)

La primera página que ve después de iniciar sesión es el panel principal. Este es su centro central para ver el estado de todos sus servicios administrados.
...
* **Tabla de reglas ingress administradas:** Esta tabla enumera todas las reglas ingress que administra DockFlare, ya sea que provengan de un contenedor Docker o se hayan creado manualmente.
    ...
    * **Fuente:** Indica si la regla es de `Docker` o se creó `Manually` en la Web UI.
...
* **Registros en tiempo real:** Debajo de la tabla, encontrará un visor de registros en tiempo real que transmite registros desde el backend de DockFlare, lo cual es invaluable para la depuración.

## Gestión de reglas

La Web UI le brinda control total sobre sus reglas ingress.
...
* **Manage Rule:** El botón "Manage Rule" al lado de cada regla abre un modal donde puede cambiar su configuración. Así es como puede aplicar un override de la Web UI a una regla que se creó originalmente a partir de etiquetas de Docker.
* **Revert to Labels:** Si una regla de Docker tiene un override de la Web UI, aparecerá el botón "Revert to Labels", que le permitirá descartar los cambios manuales y dejar que la regla vuelva a ser controlada por sus etiquetas de Docker.
...
* **Red de seguridad:** Incluso si olvida agregar una política a un servicio específico, la política comodín a nivel de zona la detectará.
...
* **Seguridad:**
    * **Change Password:** Cambie su contraseña para la Web UI.
    * **Disable Password Login:** Para casos de uso avanzados en los que coloca DockFlare detrás de otro proxy de autenticación. **⚠️ Advertencia:** Esto crea un riesgo de seguridad debido a la exposición de la red Docker: cualquier contenedor en la misma red Docker puede eludir la autenticación externa y acceder a la API de DockFlare directamente. Recomendamos encarecidamente utilizar proveedores de OAuth/OIDC para obtener la comodidad del single sign-on sin sacrificar la seguridad. Consulte [Acceso a la Web UI](Accessing-the-Web-UI.md) para conocer todas las implicaciones de seguridad.
