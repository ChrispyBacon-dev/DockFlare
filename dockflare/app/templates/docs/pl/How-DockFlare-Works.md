...
| --- | --- |
| DockFlare Master | Udostępnia interfejs użytkownika i API, obserwuje zdarzenia Dockera oraz orkiestruje tunele Cloudflare, DNS i zasady dostępu. Działa bez uprawnień roota i komunikuje się z Dockerem wyłącznie przez socket proxy. |
...
| Redis | Cache, kolejki, strumieniowanie logów oraz kanał heartbeat/zwrotny agentów. Działa w prywatnej sieci `dockflare-internal`. |
...
2. **Grupy dostępu (warstwa abstrakcji)**: aby nie powtarzać złożonych polityk dostępu w wielu usługach, możesz tworzyć wielokrotnego użytku **Grupy dostępu** w Web UI. Są to szablony grupujące zestawy reguł, takie jak dostęp dla firmowych adresów e-mail albo z określonych krajów, które synchronizują się z nazwanymi politykami Cloudflare Access. Przełącznik Public vs Authenticated w oknie dialogowym decyduje, czy DockFlare zastosuje `bypass`, czy `allow`. Następnie całą politykę można przypisać do kontenera jedną etykietą, na przykład `dockflare.access.group=my-policy-group`.

3. **Nadpisania w Web UI (warstwa kontroli)**: Web UI daje najwyższy poziom kontroli. Z panelu możesz:
...
Ten model warstwowy pozwala zautomatyzować większość usług za pomocą etykiet Dockera, a jednocześnie zachować możliwość obsługi wyjątków i bardziej złożonych scenariuszy z poziomu Web UI.
...
Polityki wildcard na poziomie strefy (`*.domain.com`) zapewniają warstwowe bezpieczeństwo dzięki priorytetom:

1. **Polityka dla konkretnego hosta** (na przykład `app.example.com`) - najwyższy priorytet
2. **Polityka wildcard strefy** (na przykład `*.example.com`) - zabezpieczenie zapasowe
