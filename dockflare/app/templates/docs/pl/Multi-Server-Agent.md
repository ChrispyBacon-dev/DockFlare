# DockFlare Agent i architektura wieloserwerowa

DockFlare 3.0 wprowadza rozproszony model działania, który pozwala zarządzać tunelami Cloudflare na wielu hostach Docker. **Master** DockFlare koordynuje konfigurację, a lekcy **Agenci** działają obok Twoich obciążeń i utrzymują lokalną instancję `cloudflared` w synchronizacji.
...
* **Oddzielenie obliczeń od ingressu** – utrzymuj obciążenia blisko użytkowników, zachowując jedną płaszczyznę sterowania.
* **Widoczność per host** – monitoruj heartbeat, stan tunelu i historię poleceń dla każdego agenta.
...
|-----------|----------------|
| **Master (DockFlare)** | Udostępnia Web UI, przechowuje stan, uzgadnia docelowe reguły ingressu i wydaje polecenia. |
| **Redis** | Backplane dla cache, heartbeatów agentów i poleceń w kolejce. |
| **DockFlare Agent** | Kontener bez interfejsu, który obserwuje lokalne zdarzenia Dockera, wykonuje polecenia i uruchamia `cloudflared`. |
...
1. **Wygeneruj klucz API agenta** w interfejsie DockFlare (`Agents → Generate Key`).
2. **Wdróż kontener DockFlare Agent** na zdalnym hoście, przekazując adres URL Mastera i klucz.
3. Agent **rejestruje się** w Masterze i pojawia się ze statusem *Pending*.
4. Z poziomu interfejsu Mastera **zarejestruj** agenta i przypisz mu lub utwórz tunel Cloudflare dla tego hosta.
5. Master umieszcza polecenia w kolejce, a agent **odpytuje**, stosuje konfigurację i raportuje stan oraz heartbeat. DockFlare automatycznie wykrywa strefę docelową dla każdej nazwy hosta i wraca do strefy domyślnej tylko wtedy, gdy wykrywanie się nie powiedzie.
6. Gdy kontenery są uruchamiane lub zatrzymywane na hoście agenta, agent przesyła zdarzenia z powrotem do Mastera, który aktualizuje DNS, polityki Access i reguły ingressu tunelu.

---

## Wdrażanie DockFlare Agent
...
2. **Nie wyłączaj logowania hasłem**. Zamiast tego użyj dostawców OAuth/OIDC, aby zapewnić wygodę single sign-on bez ryzyka dla bezpieczeństwa. Jeśli musisz wyłączyć logowanie hasłem, pamiętaj, że tworzy to lukę w zabezpieczeniach sieci Docker: każdy kontener w tej samej sieci może ominąć zewnętrzne uwierzytelnianie. Zobacz [Dostęp do Web UI](Accessing-the-Web-UI.md), aby poznać pełne konsekwencje bezpieczeństwa.
3. Używaj oddzielnych tuneli dla każdego agenta, aby zachować izolację minimalnych uprawnień.
4. Monitoruj stronę `Agents` pod kątem przerw w heartbeatach; węzły offline można usunąć bezpośrednio z interfejsu.
...
* Przejrzyj zaktualizowany Quick Start w README repozytorium, aby upewnić się, że Redis jest skonfigurowany.
* Sprawdź changelog pod kątem zmian niekompatybilnych i uwag migracyjnych.
* Zasubskrybuj publiczne repozytorium DockFlare Agent po jego opublikowaniu, aby być na bieżąco z wydaniami.
