...
Sama aplikacja DockFlare ma Web UI. Aby je chronić i zapewnić jego bezpieczeństwo, DockFlare wdraża ścisłą politykę bezpieczeństwa treści (CSP) we własnym interfejsie.
...
Ważne jest, aby zrozumieć, że CSP DockFlare ma zastosowanie **tylko do Web UI DockFlare**.
...
CSP DockFlare stanowi integralną część jego profilu bezpieczeństwa i **nie jest konfigurowalny przez użytkownika**. Zasady są tak dobrane, aby były możliwie restrykcyjne, a jednocześnie umożliwiały prawidłowe działanie Web UI.
