...
|ラベル |説明 |例 |
| :--- | :--- | :--- |
| `dockflare.enable` | **必須。** マスタースイッチ。DockFlare がこのコンテナを管理するには `true` に設定する必要があります。 | `dockflare.enable=true` |
...
このワークフローを推奨するため、クイック作成オプションは UI から削除されました。
...
**推奨:** セキュリティ強化のため、UI で全ドメインの Zone Default Policy を作成してください。

|ラベル |説明 |例 |
| :--- | :--- | :--- |
| `dockflare.access.group` | このサービスに適用する Access Group の ID。ID は DockFlare UI の Access Policies ページで確認できます。 | `dockflare.access.group=internal-tools-policy` |
