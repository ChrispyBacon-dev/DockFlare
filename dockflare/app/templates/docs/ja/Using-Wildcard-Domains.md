...
DockFlare は引き続き Cloudflare Tunnel 内の **ingress ルール** は管理しますが、最初の DNS セットアップだけは手動です。
...
2. DockFlare がコンテナを検出し、Cloudflare Tunnel 内に `*.plex.example.com` の ingress ルールを作成します。
3. DockFlare Web UI と Cloudflare ダッシュボードのトンネル設定で確認できます。
