...
- **Protezione delle API del Master** – Gli endpoint amministrativi (Web UI, `/api/v2/*`) richiedono una sessione valida o la chiave API del Master. I token vengono oscurati nelle risposte e nei log e possono essere ruotati senza riavviare lo stack.

## 2. Configurazione cifrata e gestione delle chiavi

- **`dockflare_config.dat` cifrato** – Le credenziali Cloudflare, gli account della Web UI, i valori predefiniti dei tunnel e la chiave master vengono salvati in un blob cifrato protetto da `dockflare.key`.
...
- **Backplane Redis** – DockFlare usa Redis per cache, streaming dei log e segnalazione cross-thread. Lo stack Compose consigliato mantiene Redis su una rete dedicata `dockflare-internal`, così i workload su `cloudflare-net` non possono raggiungerlo direttamente. Se usi un servizio Redis esterno, proteggilo con autenticazione e TLS.
...
- **Accesso alla Web UI protetto** – Il wizard iniziale impone la creazione di un account amministratore per la Web UI. L'accesso con password può essere disabilitato, ma **lo sconsigliamo fortemente** per le implicazioni di sicurezza sulla rete Docker.
...
| Monitoraggio | Genera avvisi per riavvii inattesi, heartbeat agent mancanti o nuova emissione di chiavi fuori dalle finestre di manutenzione. |
