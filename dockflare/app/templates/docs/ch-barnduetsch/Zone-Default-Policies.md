...
Zonen-Standardrichtlinien si ein Security-Best-Practice-Feature, das Cloudflare Access Wildcard-Applications (`*.domain.com`) nutzt, um alle Subdomains einer DNS-Zone automatisch zu schützen.

## Das Problem, das dies löst

Ohne Zonen-Standardrichtlinien:
- bleiben vergessene Services öffentlich erreichbar
- si neue Subdomains ungeschützt, bis sie manuell konfiguriert wärde
- chöi Tippfehler in Hostnamen-Konfigurationen Zugriffskontrollen umgehen
...
3. **Keine Übereinstimmung** = öffentlich (kener Access App)
...
- lässt du auswählen, welche Access Group die Zone schützt
...
2. Scroll zu **Zone Default Policies (*.tld Wildcards)**
3. Prüef den Schutzstatus:
   - 🛡️ **Grün "Protected"**: Zone hat eine Wildcard-Policy
   - ⚠️ **Gelb "Not Protected"**: Zone isch angreifbar

### Schritt 2: Zonen-Policies erstellen

Für jede ungeschützte Zone:

1. Klick auf **Create Policy**
2. Das Modal zeigt den Hostnamen `*.zone-name.com`
3. Wähl eine passende Access Policy:
   - **Öffentliche Zonen**: `public-default-bypass`
   - **Interne Zonen**: Authentifizierungs-Policy
   - **Gemischte Zonen**: die restriktivste Policy
4. Klick auf **Create Zone Policy**

### Schritt 3: In Cloudflare verifizieren

1. Mach uf das Cloudflare Zero Trust Dashboard
2. Navigier zu Access → Applications
3. Suech nach Applications mit dem Namen `Zone Default: *.domain.com`
4. lueg dass die Policy korrekt isch

## Sicherheitsempfehlungen

### Produktionsumgebungen

✅ **Aktivier Zonen-Standardrichtlinien immer**
...
Regelmässig prüfen:
- welche Zonen dr Schutz aktiv hesch (Siite **Access Policies**)
...
**Lösig:** Verwalt si direkt i Cloudflare, oder lösch si u erstell si über DockFlare neu.

### Service isch weiterhin ohne Authentifizierung erreichbar

Prüef d'Policy-Priorität:
1. Lueg, dass es e spezifischi Hostname-Policy für dä Service git
2. Bestätig, dass die Zonen-Wildcard existiert u korrekt konfiguriert isch
3. Wenn der Service trotz Zonenschutz öffentlich bleiben soll, setz `dockflare.access.group=public-default-bypass`

### Zonenschutz für öffentliche Services umgehen

Wänn du e Zonen-Policy hesch, aber einzeli Services öffentlich blibe müesse:

1. Füg dem Container das Bypass-Label hinzu:
   ```yaml
   labels:
...
### Zone wird nid in der Liste angezeigt

Mögliche Ursachen:
- Zone existiert nid in dim Cloudflare-Account
- API-Token hat kener `Zone:Zone:Read` Berechtigung
- Zone isch pausiert oder gelöscht

**Lösung:** Prüef im Cloudflare Dashboard, öb die Zone existiert u öb das API-Token die nötigen Rechte hat.

## Best Practices

1. **Zonen-Policies zuerst erstellen**: bevor du Services hinzufügen
2. **Interne Zonen authentifizieren**: niemals `bypass` für interne Zonen bruuche
3. **Ausnahmen dokumentieren**: wenn eine Zone kei Schutz braucht, dokumentier warum
4. **Regelmässige Audits**: monatliche Prüfung des Schutzstatus
5. **Vor Produktion testen**: sicherstellen, dass die Wildcard-Policy nichts bricht
6. **Least Privilege**: die restriktivste Policy bruuche, die legitimen Zugriff noch erlaubt
...
**Setup:**
- Zonen-Policy: `*.example.com` → Authentifizierung nötig
- Spezifische Policy: `blog.example.com` → `public-default-bypass`

**Ergebnis:**
- `blog.example.com` → öffentlich (spezifische Policy gewinnt)
- `api.example.com` → Auth nötig (Wildcard greift)
- `forgotten.example.com` → Auth nötig (Wildcard greift)

### Szenario 2: Wildcard als Safety Net

**Setup:**
- Zonen-Policy: `*.internal.company.com` → erfordert @company.com E-Mail
- Spezifische Policy: kener für `test-server.internal.company.com`

**Ergebnis:**
- `test-server.internal.company.com` → Auth nötig (Wildcard schützt)
- selbst wenn du es vergessen: die Zonen-Policy schützt es

### Szenario 3: Kein Schutz

**Setup:**
- Zonen-Policy: kener für `*.risky-domain.com`
- Spezifische Policy: `app.risky-domain.com` → Authentifizierung

**Ergebnis:**
- `app.risky-domain.com` → Auth nötig (spezifische Policy)
- `forgotten.risky-domain.com` → ⚠️ **PUBLIC** (kener Wildcard fängt es ab)

## Integration mit DockFlare-Labels

### Verwendung des Labels `default_tld`

Das Label `dockflare.access.policy=default_tld` weist DockFlare an, die Wildcard-Policy der Zone zu bruuche:
...
- kener Zonen-Policy → Service isch öffentlich (kener Access App wird erzeugt)
